    ...
    private void handleConfirmBooking() {
        try {
            if (!utils.Session.isLoggedIn()) {
                throw new IllegalStateException("Bạn cần đăng nhập để đặt vé!");
            }
            ...
            if (isSuccess) {
                ...
                throw new RuntimeException("Lỗi I/O khi lưu file CSDL.");
            }

        } catch (IllegalStateException ex) {
            JOptionPane.showMessageDialog(this, ex.getMessage(), "Cảnh báo", JOptionPane.WARNING_MESSAGE);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Hệ thống bảo trì: " + ex.getMessage(), "Lỗi nghiêm trọng", JOptionPane.ERROR_MESSAGE);
        }
    }
